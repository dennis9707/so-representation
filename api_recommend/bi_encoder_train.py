...
model_name_simple = args.model_name.replace('/', '_')
print('model_name_simple',model_name_simple)
...
for p in [15]:
  for n in [15]:
    ...
    model_save_path = './SBRT_output/'+ model_name_simple  + '_batchsize_' + str(args.batch_size)  + '-bi-encoder-min_5_max_'+p+'_ir_'+n+'_5k'+'-'+datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    corpus = {}
    ...
    ir_evaluator = evaluation.InformationRetrievalEvaluator(dev_queries, corpus, dev_rel_docs, name=model_name_simple+'-train_eval')
    ...
    model.fit(train_objectives=[(train_dataloader, train_loss)],
              ...
              checkpoint_save_steps=7000,
              checkpoint_path=model_save_path+'/process/',
              use_amp=True)
